...
export const capture11: GameModule = {
  ...
  mount(root, services): MountedGame {
    ...
    const makeCpuPlayPanel = (play: CpuPlayPresentation, preview: boolean): HTMLElement => {
      ...
      panel.className = `cpu-play-panel${preview ? ' is-preview' : ''}`;
      ...
      const card = document.createElement('span'); card.className = `playing-card cpu-play-card ${cardColorClass(play.card)}`; card.textContent = cardText(play.card);
      card.setAttribute('aria-label', `${play.card.rank} of ${SUIT_NAME[play.card.suit]}`);
      const text = document.createElement('div');
      const heading = document.createElement('h3'); heading.textContent = play.heading;
      const detail = document.createElement('p'); detail.textContent = play.detail;
      text.append(heading, detail); panel.append(label, card, text); return panel;
    };

    const makeDiagnostics = (): HTMLDetailsElement => {
      const diagnostics = document.createElement('details'); diagnostics.className = 'playtest-diagnostics';
    ...
    const scheduleCpu = () => {
      ...
      cpuTimer = window.setTimeout(() => {
        ...
        cpuPreview = describeCpuMove(state, move);
        render();
        cpuTimer = window.setTimeout(() => {
          cpuTimer = null;
          if (destroyed || state.phase !== 'playing' || state.turn !== 'player2') return;
          const shown = cpuPreview;
          applyRecordedMove('player2', move);
          lastCpuPlay = shown;
          cpuPreview = null;
          clearSelection();
          render();
    ...
    const renderScoreScreen = () => {
      ...
      const grid = document.createElement('div'); grid.className = 'score-breakdown';
      const labels = document.createElement('div'); labels.className = 'score-row score-head'; labels.append(document.createElement('span'), Object.assign(document.createElement('strong'), { textContent: 'You' }), Object.assign(document.createElement('strong'), { textContent: 'CPU' }));
      ...
      const last = document.createElement('p'); last.className = 'last-action'; last.textContent = state.lastAction;
    ...
    const render = () => {
      ...
      const header = document.createElement('header'); header.className = 'capture11-header';
      const titleWrap = document.createElement('div'); const heading = document.createElement('h1'); heading.textContent = 'Capture 11'; const subtitle = document.createElement('p'); subtitle.textContent = `Hand ${state.handNumber} · First to 11`; titleWrap.append(heading, subtitle);
      ...
      const playToShow = cpuPreview ?? lastCpuPlay; if (playToShow) cpuArea.append(makeCpuPlayPanel(playToShow, cpuPreview !== null));
      ...
      for (const item of state.board) {
        ...
        if (item.kind === 'loose') { button.textContent = cardText(item.card); button.setAttribute('aria-label', `${item.rank ?? ''}${cardText(item.card)} on board`); button.setAttribute('aria-label', `${item.card.rank} of ${SUIT_NAME[item.card.suit]} on board`); }
      ...
      if (state.turn !== 'player1') { const waiting = document.createElement('p'); waiting.textContent = cpuPreview ? 'CPU card is revealed above. Board update is paused so you can inspect the play.' : 'CPU is thinking…'; actionPanel.append(waiting); }
        ...
        if (exactMoves.length === 0) { const invalid = document.createElement('p'); invalid.textContent = 'No legal action for that exact selection. Change the board selection, or clear it to trail the card.'; actionPanel.append(invalid); }
        else { const actions = document.createElement('div'); actions.className = 'action-buttons'; for (const move of exactMoves) actions.append(makeButton(actionLabel(move, state), () => playMove(move), move.type.startsWith('capture') ? 'primary-action' : '')); actionPanel.append(actions); }

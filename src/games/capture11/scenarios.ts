...
export interface Capture11Scenario {
  ...
  readonly expected: Capture11Move | readonly Capture11Move[];
...
const build = (id: string, target: number, createdBy: PlayerId, cards: Card[]): BoardItem =>
  ({ kind: 'build', id, target, createdBy, cards, components: [cards], mode: 'open' });
...
function state(parts: StateParts): Capture11State {
  const cpu = parts.cpu ?? [card('J', 'diamonds'), card('6', 'clubs'), card('3', 'clubs'), card('10', 'clubs')];
  const captured = parts.captured ?? []; const cpuCaptured = parts.cpuCaptured ?? [];
  const boardCards = parts.board.flatMap(item => item.kind === 'loose' ? [item.card] : item.cards);
  const used = new Set([...parts.hand, ...cpu, ...boardCards, ...captured, ...cpuCaptured].map(value => value.id));
  const drawDeck = parts.deck ?? deck.filter(value => !used.has(value.id));
  return {
    seed: 110011, dealer: parts.dealer ?? 'player2', turn: parts.turn ?? 'player1', deck: drawDeck,
    board: parts.board,
    players: {
      player1: { id: 'player1', hand: parts.hand, captured, matchScore: parts.matchScore ?? 0 },
      player2: { id: 'player2', hand: cpu, captured: cpuCaptured, matchScore: parts.cpuMatchScore ?? 0 },
    },
...
function scenario(definition: Omit<Capture11Scenario, 'suggestedHandCardId' | 'suggestedBoardKeys'>): Capture11Scenario {
  const expected = (Array.isArray(definition.expected) ? definition.expected[0]! : definition.expected) as Capture11Move;
  const boardKeys = expected.type === 'capture-build' || expected.type === 'raise-build'
    ? [`build:${expected.buildId}`]
    : expected.type === 'extend-build'
      ? [`build:${expected.buildId}`, ...expected.cardIds.map(id => `loose:${id}`)]
...
export function loadScenario(id: ScenarioId): Capture11Scenario {
  ...
  const finalLoose = [finalBoard, card('K', 'clubs'), card('Q', 'diamonds'), card('6', 'spades')];
  const finalMustHave = [card('A', 'spades'), card('2', 'spades'), card('10', 'diamonds'), card('3', 'spades'), card('5', 'spades')];
  const finalReserved = new Set([finalHand, ...finalLoose, ...finalMustHave].map(value => value.id));
  const finalRemainder = deck.filter(value => !finalReserved.has(value.id));
  const finalPlayerCaptured = [...finalMustHave, ...finalRemainder.slice(0, 18)];
  const finalCpuCaptured = finalRemainder.slice(18);
  const multiBuildHand = card('10', 'spades'); const multiBuildA = card('6', 'hearts'); const multiBuildB = card('4', 'clubs');
  const multiBuildC = card('7', 'diamonds'); const multiBuildD = card('3', 'spades');

  const scenarios: Record<ScenarioId, Capture11Scenario> = {
    basic_capture: scenario({ id: 'basic_capture', title: '1 of 7 · Basic Capture',
      ...
      state: state({ hand: [basicHand, card('7', 'diamonds'), card('Q', 'clubs'), card('9', 'hearts')], board: [loose(basicBoard), loose(card('K', 'clubs')), loose(card('2', 'diamonds')), loose(card('6', 'spades'))] }),
      expected: { type: 'capture-loose', handCardId: basicHand.id, cardIds: [basicBoard.id] } }),
    bug_004_multi_equal_capture: scenario({ id: 'bug_004_multi_equal_capture', title: '2 of 7 · Multiple Equal Cards',
      ...
      state: state({ hand: [bugHand, card('7', 'diamonds'), card('Q', 'clubs'), card('9', 'hearts')], board: [loose(bugA), loose(bugB), loose(card('K', 'clubs')), loose(card('Q', 'hearts')), loose(card('2', 'diamonds'))] }),
      expected: { type: 'capture-loose', handCardId: bugHand.id, cardIds: [bugA.id, bugB.id] } }),
    build_demo: scenario({ id: 'build_demo', title: '3 of 7 · Make a Build',
      ...
      state: state({ hand: [buildHand, card('5', 'spades'), card('Q', 'clubs'), card('9', 'hearts')], board: [loose(buildBoard), loose(card('K', 'clubs')), loose(card('2', 'diamonds')), loose(card('6', 'spades'))] }),
      expected: { type: 'build-open', handCardId: buildHand.id, cardIds: [buildBoard.id], target: 5 } }),
    build_takeover_demo: scenario({ id: 'build_takeover_demo', title: '4 of 7 · Take Over a Build',
      ...
      state: state({ hand: [takeoverHand, card('8', 'spades'), card('Q', 'clubs'), card('9', 'hearts')], board: [build('demo-build-5', 5, 'player2', [card('A', 'diamonds'), card('4', 'clubs')]), loose(card('Q', 'hearts')), loose(card('2', 'diamonds')), loose(card('6', 'spades'))] }),
      expected: { type: 'raise-build', handCardId: takeoverHand.id, buildId: 'demo-build-5', target: 8 } }),
    strategy_demo: scenario({ id: 'strategy_demo', title: '5 of 7 · Capture More in One Play',
      ...
      state: state({ hand: [strategyHand, card('7', 'diamonds'), card('Q', 'clubs'), card('9', 'clubs')], board: [loose(strategyTen), loose(strategyNine), loose(strategyAce), loose(card('K', 'clubs')), loose(card('6', 'spades'))] }),
      expected: { type: 'capture-loose', handCardId: strategyHand.id, cardIds: [strategyTen.id, strategyNine.id, strategyAce.id] } }),
    final_sweep_demo: scenario({ id: 'final_sweep_demo', title: '6 of 7 · Final Sweep and Scoring',
      instruction: 'The deck and CPU hand are empty, and this is your last card. Capture the marked table 4; as final capturer, you also receive the remaining K, Q, and 6 before normal scoring runs.',
      ...
      isComplete: next => next.phase === 'hand-over' && next.board.length === 0 && next.lastHandScore !== null,
      state: state({ hand: [finalHand], cpu: [], deck: [], board: finalLoose.map(loose),
        captured: finalPlayerCaptured, cpuCaptured: finalCpuCaptured, lastCapturer: 'player2' }),
      expected: { type: 'capture-loose', handCardId: finalHand.id, cardIds: [finalBoard.id] } }),
    multi_build_capture_demo: scenario({ id: 'multi_build_capture_demo', title: '7 of 7 · Multi-Build Capture',
      instruction: 'First select your 10 and the marked BUILD 10 plus 7♦ and 3♠, then choose Add 10 component. Your 10 stays in hand. Select it again with the completed build and capture the entire build.',
      success: 'The complete multi-component BUILD 10 was captured as one unit. Every component card moved to your captured pile.',
      isComplete: next => next.board.every(item => item.kind !== 'build') && [multiBuildHand, multiBuildA, multiBuildB, multiBuildC, multiBuildD].every(value => next.players.player1.captured.some(captured => captured.id === value.id)),
      state: state({ hand: [multiBuildHand, card('K', 'clubs'), card('5', 'hearts'), card('2', 'clubs')], board: [build('multi-build-10', 10, 'player1', [multiBuildA, multiBuildB]), loose(multiBuildC), loose(multiBuildD), loose(card('Q', 'hearts')), loose(card('9', 'clubs'))] }),
      expected: [
        { type: 'extend-build', handCardId: multiBuildHand.id, buildId: 'multi-build-10', cardIds: [multiBuildC.id, multiBuildD.id], target: 10 },
        { type: 'capture-build', handCardId: multiBuildHand.id, buildId: 'multi-build-10' },
      ] }),
  };
  return scenarios[id];
}

export function isExpectedScenarioMove(actual: Capture11Move, expected: Capture11Move | readonly Capture11Move[]): boolean {
  ...
  const expectedMoves = Array.isArray(expected) ? expected : [expected];
  return expectedMoves.some(candidate => normalize(actual) === normalize(candidate));
}

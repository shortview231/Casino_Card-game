...
const HERO_SUIT_SYMBOL: Readonly<Record<PlayingCardVisual['suit'], string>> = {
  spades: '♠',
  hearts: '♥',
  diamonds: '♦',
  clubs: '♣',
...
function makeHeroCards(): HTMLElement {
  ...
  for (const card of HERO_CARDS) {
    ...
    host.append(text('span', HERO_SUIT_SYMBOL[card.suit], 'c11-menu-card-suit'));
    cards.append(host);
  }
  return cards;
}

function makeHeroTableau(): HTMLElement {
  const tableau = document.createElement('div');
  tableau.className = 'c11-menu-tableau';
  const deck = document.createElement('div');
  deck.className = 'c11-menu-deck';
  deck.setAttribute('aria-hidden', 'true');
  for (let index = 0; index < 4; index += 1) {
    const card = document.createElement('span');
    card.className = 'c11-menu-deck-card';
    renderPlayingCardBack(card, 'Face-down Capture 11 card');
    card.setAttribute('aria-hidden', 'true');
    deck.append(card);
  }
  tableau.append(makeHeroCards(), deck);
  return tableau;
}
...
export function renderCapture11MainMenu(root: HTMLElement, actions: GameTitleActions): void {
  ...
  const showHowToPlay = () => {
    ...
    const rules = [
      ...
      'Open builds can be raised or burned. Locked builds contain two or more groups equal to one target; they cannot be raised, but another complete target group may be added while its pickup card remains held.',
